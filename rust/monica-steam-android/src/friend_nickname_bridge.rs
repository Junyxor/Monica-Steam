...
const FRIEND_NICKNAME_BRIDGE_MAGIC: &[u8; 4] = b"MSN1";
...
fn serialize_friend_nicknames(items: &[FriendNickname]) -> Option<Vec<u8>> {
    ...
    let mut out = Vec::new();
    out.extend_from_slice(FRIEND_NICKNAME_BRIDGE_MAGIC);
...
mod tests {
    ...
    #[test]
    fn friend_nickname_bridge_layout_is_stable() {
        let encoded = serialize_friend_nicknames(&[
            FriendNickname {
        ...
        assert_eq!(u32::from_le_bytes(encoded[16..20].try_into().unwrap()), 5);
        assert_eq!(&encoded[20..25], b"Alice");
    }
}
